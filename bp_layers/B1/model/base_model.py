...
class BaseLLMProvider:
    """
    Wrapper haut niveau utilisé par les nodes BPACC.
...
def reformulation_llm(system_prompt=None):
    return BaseLLMProvider(system_prompt=system_prompt, temperature=0.4)

def generation_llm(system_prompt=None):
    return BaseLLMProvider(system_prompt=system_prompt, temperature=0.2)


if __name__ == "__main__":
    import asyncio

    print("=== BPACC LLM Provider — Test rapide ===\n")
    ...
    print(llm_json.invoke_for_json('Retourne {"nom": "BPACC", "version": 1}'))
